  ...
  q <- ncol(x)
  l <- function(beta) {
      betamu <- beta[1:q]; betasig <- beta[q + 1]; betash <- beta[q + 2]
      mu <- x %*% betamu
      sig <- exp(betasig)
      sh <- betash
      ll <- as.numeric()
      ...
      # standard likelihood  
        for (i in 1:n) {
          ll[i] <- dgev(y[i], mu[i], sig, sh, log = TRUE)
        }  
        ans <- sum(ll)  
      }
      return(ans)
  ...
  init <- gev.fit(y)$mle
  start <- c(init[1], rep(0, q - 1), log(init[2]), init[3])
  mle <- optim(start, l, hessian = TRUE, method = "BFGS", control = list(fnscale = -1))
  betahat <- mle$par
  ...
  burn <- 1000
  T <- 10000
  betas <- matrix(0, nrow = T, ncol = q + 2)
  betas[1, ] <- betahat
  c <- 10^2
  set.seed(1)
...
betasig <- betas[(burn+1):T, q + 1]
sig <- exp(betasig)
lbandsig <- quantile(sig, 0.025) 
bmediansig <- median(sig)
ubandsig <- quantile(sig, 0.975) 
# plot fit and data
...
psig <- ggplot(dfsig, aes(x = date, y = y)) +
  ...
  scale_y_continuous(name = expression(sigma[t]), breaks = seq(0.4, 0.8, by = 0.1), 
                     limits = c(0.4, 0.8)) + 
  ggtitle("b) Scale and Shape")

# plot fit and data
betash <- betas[(burn + 1):T, q + 2] 
lbandsh <- quantile(betash, 0.025) 
bmediansh <- median(betash)
ubandsh <- quantile(betash, 0.975) 
...
psh <- ggplot(dfsh, aes(x = date, y = y)) +
  ...
  scale_y_continuous(name = expression(xi[t]), breaks = seq(-0.25, 0.15, by = 0.1), 
                     limits = c(-0.25, 0.15)) 
...
e <- numeric()
for (i in 1:n)
  e[i] <- qnorm(pgev(y[i], bmedianmu[i], bmediansig, bmediansh))
ggplot(data = data.frame(y = e), aes(y = y)) + 
  geom_qqboxplot(notch = TRUE, varwidth = TRUE, reference_dist = "norm") + 
  ylab('Normal Scale') + 
  ggtitle('Hong Kong Temperature') + 
  theme_minimal() +
...
# COMPUTE WAIC
require(loo)
# (L is log-likelihood matrix where each column is an observation and 
# each line is an iteration)
L1 <- matrix(NA, nrow = T - burn, ncol = n)
for(t in 1:(T - burn))
  for(i in 1:n)
    L1[t, i] <- log(dgev(y[i], x[i, ] %*% betamu[t, ], exp(betasig[t]), betash[t]))
model.eval1 <- waic(L1)
model.eval1$estimates
WAIC <- -2 * (model.eval1$estimates[1, 1])