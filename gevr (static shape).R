  ...
  q <- ncol(x)
  l <- function(beta) {
      betamu <- beta[1:q]; betasig <- beta[(q+1):(2*q)]; betash <- beta[(2*q+1)]
      mu <- x %*% betamu
      sig <- exp(x %*% betasig)
      sh <- betash
      ll <- as.numeric()
      ...
      # standard likelihood  
        for (i in 1:n) {
          ll[i] <- dgev(y[i], mu[i], sig[i], sh, log = TRUE)
        }  
        ans <- sum(ll)  
      }
      return(ans)
  ...
  init <- gev.fit(y)$mle
  start <- c(init[1], rep(0, q - 1), log(init[2]), rep(0, q - 1), init[3])
  ...
  burn <- 1000
  T <- 10000
  betas <- matrix(0, nrow = T, ncol = 2 * q + 1)
  betas[1, ] <- betahat
  c <- 10^2
  set.seed(1)
...
psig <- ggplot(dfsig, aes(x = date, y = y)) +
  ...
  #labs(x = "Time (years)", y = expression(sigma[t]), title = "")

betash <- betas[(burn + 1): T,(2*q+1)] 
...
for(i in 1:(T - burn))
  for(j in 1:n)
    traj.tvq[i, j] <-  qgev(0.99, x[j, ] %*% betamu[i, ], 
                            exp(x[j, ] %*% betasig[i, ]), betash[i])
...
L2 <- matrix(NA, nrow = T - burn, ncol = n)
for(t in 1:(T - burn))
  for(i in 1:n)
    L2[t, i] <- log(dgev(y[i], x[i, ] %*% betamu[t, ], 
                        exp(x[i, ] %*% betasig[t, ]), betash[t]))
model.eval2 <- waic(L2)
model.eval2$estimates
WAIC <- -2 * (model.eval2$estimates[1, 1])