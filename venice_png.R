...
plot_tenmax <- function() {
  fig <- ggplot() +
    ...
    geom_line(aes(x+1900,fitted(fit.gam)[,1]),col="darkgrey")+
    geom_point(data = tidyr::gather(ten_yearly_max_venice_1887_2023, max_n, level_cm, -year), 
               aes(x = year, y = level_cm), color = "black", pch = 16, cex = 0.7) +
    theme_classic(base_size=11) +
    theme(axis.text = element_text(size = 10), panel.background = element_rect(fill = "white",
                                colour = "white",
                                size = 0.5, linetype = "blank"))
  ggsave("figures/venice1.png", bg = "white", unit = 'px', width = 2000, height = 1000, plot=fig)
}
...
plot_gam_fit <- function (){
  fit1 <- fgev(x=y,nsloc=x) # straight-line regression
  print(fit1)  # output results of this fit
  muhat <- fit1$estimate[1] + fit1$estimate[2]*x
  ...
  plot <- ggplot(data=data.frame(qq), aes(x=x))+
    ...
    theme(axis.text = element_text(size = 10),
          panel.background = element_rect(fill = "white",
                                colour = "white",
                                size = 0.5, linetype = "blank"))
  junk <- make.exp.res(fit=fitr3, data=ten_yearly_max_venice_1887_2023[ten_yearly_max_venice_1887_2023$year<2020, ], 
                       r=3, yl=yl)
  plot_list <- c(list(plot), junk)
  total <- cowplot::plot_grid(plotlist = plot_list,
                                 labels = c("", ""),
                                 ncol = 2, nrow=2)
  ggsave("figures/venice2.png", bg = "white", dpi = 200, unit = 'mm', width = 160, height = 140,plot=total)
